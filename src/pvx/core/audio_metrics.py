...
def summarize_audio_comparison_metrics(
    ...
    def _si_sdr_db(reference: np.ndarray, degraded: np.ndarray) -> float:
        ...
        return float(
            10.0
            * np.log10((float(np.dot(target, target)) + eps) / (float(np.dot(noise, noise)) + eps))
        )
    ...
    if bins == 0 or frames == 0:
        ...
        if m_bins > 0:
            ...
            modspec_output = float(
                np.sqrt(np.mean(m_diff * m_diff)) / (np.mean(np.abs(ref_mod[:, :m_bins])) + eps)
            )
        else:
            modspec_output = float("nan")
    ...
    def _integrated_lufs(signal: np.ndarray, sample_rate: int) -> float:
        try:
            ...
            return float(
                meter.integrated_loudness(np.asarray(signal, dtype=np.float64).reshape(-1))
            )
    ...
    if ref_2d.shape[1] >= 2 and cand_2d.shape[1] >= 2:
        ...
        b2 = min(
            spec_ref_l.shape[0], spec_ref_r.shape[0], spec_cand_l.shape[0], spec_cand_r.shape[0]
        )
        f2 = min(
            spec_ref_l.shape[1], spec_ref_r.shape[1], spec_cand_l.shape[1], spec_cand_r.shape[1]
        )
        if b2 > 0 and f2 > 0:
    ...
    *,
    ...
    rows = [
        ...
        [
            ...
            _format_float(_delta("spectral_convergence_output", "spectral_convergence_input"), 4),
            "lower",
        ],
        [
            "Envelope Corr",
        ...
        [
            ...
            _format_float(_delta("stereo_phase_drift_output", "stereo_phase_drift_input"), 4),
            "lower",
        ],
        [
            "ILD Drift dB",
