...
def write_math_foundations() -> None:
    ...
    lines.append("")
    lines.append("$$")
    lines.append("y[n]=\\frac{1}{M}\\sum_{m=1}^{M}x_m[n-\\tau_m]")
    lines.append("$$")
    lines.append("")
    lines.append("MVDR beamforming:")
    lines.append("")
    lines.append("$$")
    lines.append("\\mathbf{w}=\\frac{\\mathbf{R}^{-1}\\mathbf{d}}{\\mathbf{d}^H\\mathbf{R}^{-1}\\mathbf{d}}")
    lines.append("$$")
    lines.append("")
    lines.append("FOA ambisonic encoding:")
    lines.append("")
    lines.append("$$")
...
def write_window_reference() -> None:
    ...
    lines.append("")
    lines.append("| Window | Family | Parameters | Formula | Coherent gain | ENBW (bins) | Scalloping loss (dB) | Main-lobe width (bins) | Peak sidelobe (dB) | Plots | Plain-English interpretation |")
    lines.append("| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | --- | --- |")
    for entry in entries:
        name = entry["name"]
        m = metrics_by_name[name]
        lines.append(
            "| `{name}` | {family} | `{params}` | {formula} | {cg:.6f} | {enbw:.6f} | {scallop:.3f} | {mlw:.3f} | {psl:.3f} | [time]({time_plot}) / [freq]({freq_plot}) | {note} |".format(
                name=name,
                family=entry["family"],
                params=entry["params"],
    ...
    lines.append("")
    lines.append("## Representative Plot Gallery")
    lines.append("")
    lines.append("| Window | Time-domain shape | Magnitude spectrum |")
    lines.append("| --- | --- | --- |")
    for name in ("hann", "hamming", "blackmanharris", "flattop", "tukey_0p25", "gaussian_0p35", "kaiser", "rect"):
        lines.append(
