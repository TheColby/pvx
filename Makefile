.PHONY: install install-dev lint format typecheck test coverage docs docs-generated docs-html man pre-commit bench depsync alpha-check
...
typecheck:
	uv run python -m mypy src/pvx/core/attribution.py src/pvx/core/voc_console.py src/pvx/cli/pvxvoc.py src/pvxalgorithms scripts/scripts_apply_attribution.py
...
coverage:
	...
	uv run python -m coverage report --fail-under=65
